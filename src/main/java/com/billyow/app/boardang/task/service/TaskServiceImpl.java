...
import com.billyow.app.boardang.board.repository.IBoardMemberRepository;
...
public class TaskServiceImpl implements ITaskService{
    ...
    public TaskResponse updateTask(String taskId, UpdateTaskRequest request) {
        ...
        var task = Optional.ofNullable(taskRepository.getTaskById(taskId)).orElseThrow(() -> new ResourceNotFoundException("Task not found"));
    ...
    public TaskResponse assignCollaborator(String taskId, Long boardId, Long collaboratorId) {
        ...
        var collaboratorInBoard = memberRepository.existsByUser_IdAndBoard_Id(collaboratorId,boardId);
        if (!collaboratorInBoard) {
            throw new BadRequestException("Collaborator doesn't belong to board");
        }
    ...
    private void validateUserCanManageTasks (Board board, Long currentUserId) {
        var isOwner = boardRepository.existsByIdAndOwnerId(board.getId(), currentUserId);
        if (isOwner) return;

        var member = memberRepository.findByUser_IdAndBoard_Id(currentUserId, board.getId())
                .orElseThrow(() -> new ForbiddenException("User is not a member of this board"));
        var role = member.getRole();
        if(!(role.equals(BoardRole.ADMIN) ||  role.equals(BoardRole.MEMBER))) {
            throw new ForbiddenException("User can't modify this board");
        }
    }
}
